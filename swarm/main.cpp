...
main(int argc, const char* argv[])
{
    try
    {
        ...
        if (options->pbft_enabled())
        {
            ...
            auto crud = std::make_shared<bzn::crud>(stable_storage, std::make_shared<bzn::subscription_manager>(io_context));            auto pbft = std::make_shared<bzn::pbft>(node, io_context, peers.get_peers(), options->get_uuid()
